...
<head>
    ...
    <title>注册-个人用户</title>
    <link rel="stylesheet" type="text/css" href="${ctx}/css/headerfooter.css" />
<link rel="stylesheet" type="text/css" href="${ctx}/css/jquery.alerts.css" />
<link rel="stylesheet" type="text/css" href="${ctx}/css/headerfooterindex.css" />
<script type="text/javascript" src="${ctx}/js/jquery-1.5.1.min.js"></script>
<script type="text/javascript" src="${ctx}/js/jquery.cookie.js"></script>
<script type="text/javascript" src="${ctx}/js/passport.common.js?v20140531"></script>
<script type="text/javascript" src="${ctx}/js/jquery.alerts.js"></script>
<script type="text/javascript" src="${ctx}/js/png.js?v20140521"></script>
</head>
<body>
		<!-- header -->
		<div class="header">
		...
		<!--mainStart-->
						<link rel="stylesheet" type="text/css" href="${ctx}/css/reg.css?v20140432" />
<script type="text/javascript" src="${ctx}/js/allMail.js?v20140430"></script>
<script type="text/javascript" src="${ctx}/js/reg.js?v20150122"></script>
<!-- reg_main -->
<div class="tabBox clear">
    <span class="reg_hide reg_show regMr5" id="regper">个人用户</span>
    <span class="reg_hide" id="regcom" style="">企业用户</span>    <div class="login">已有账号，立即 <a href="${ctx }/page/login">登录</a></div>
</div>
<!--个人用户-->
...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "${ctx}/user/check/"+escape($("#regName").val())+"/1?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "${ctx}/user/check/"+$("#phone").val()+"/2?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		REGISTER.doSubmit();
				            		} else {
		...
		doSubmit:function() {
			$.post("${ctx}/user/register",$("#regForm_mod").serialize(), function(data){
				if(data.status == 200){
